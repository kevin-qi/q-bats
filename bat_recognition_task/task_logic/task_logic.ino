...
#define EVENT_REW_DELIVER 9
#define EVENT_DELIVER_RESET 10
...
State state_deliver(&on_deliver_enter, NULL, &on_deliver_exit);
Fsm fsm(&state_reset);
...
void on_rew_to_reset_timed() {
  err = "no rew beam break triggered within time limit";
}
...
void setup() {
 ...
 fsm.add_transition(&state_ready, &state_stim,
                    EVENT_READY_STIM,
                    NULL);
 fsm.add_transition(&state_stim, &state_ready,
                    EVENT_STIM_RESET,
                    &on_stim_to_reset);
 fsm.add_transition(&state_stim, &state_sniff,
                    EVENT_STIM_SNIFF,
                    NULL);
 fsm.add_transition(&state_sniff, &state_stim,
                    EVENT_SNIFF_RESET,
                    &on_sniff_to_reset);
 ...
 fsm.add_transition(&state_reset, &state_ready,
                    EVENT_RESET_READY,
                    NULL);           
 fsm.add_transition(&state_rew, &state_deliver,
                    EVENT_REW_DELIVER,
                    NULL);
 fsm.add_transition(&state_deliver, &state_reset,
                    EVENT_DELIVER_RESET,
                    NULL);                  
 ...
 fsm.add_timed_transition(&state_deliver, &state_reset,
                          3000,
                          NULL);               
